...
#define STRINGNAME 255
uint8_t findDomainName(uint8_t *answerbuf, uint8_t * answer_ptr, int *gotAnswer) {
    ...
    // NS record
        ...
        char ns_string[255];
        from_dns_style(answerbuf,answer_ptr,ns_string);
        if(debug)
...
int domainResolution(char domainName[STRINGNAME], char queryName[STRINGNAME]) {

    if (strcmp(domainName, queryName) == 0) {
        return 1;
    }
    return 0;
}
...
/* c$im dale reedonstructs a DNS query message for the provided hostname */
...
int main(int argc, char** argv)
{
    ...
    char rootServerIp[100];
    if(!nameserver) {
        FILE *rServer = fopen("root-servers.txt", "r");

        if(rServer == NULL) perror("Error opening file");
        else {
            if(fgets(rootServerIp, 100, rServer) != NULL) {
                fclose(rServer);
            }
            nameserver = rootServerIp;
        }
    ...
    while(!gotAnswer) {
    	for(a=0;a<answer_count+auth_count+other_count;a++) {
    		//*answer_ptr = findDomainName(answerbuf, answer_ptr, gotAnswer);
            // first the name this answer is referring to
            char string_name[255];
            ...
            if(domainResolution(hostname, string_name)) {
            printf("It's a match!\n");
            break;
            }
    	}
	...
	if(!gotAnswer) printf("Host %s not found.\n",argv[2]);
